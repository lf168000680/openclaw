...
export function renderDebug(props: DebugProps) {
  ...
  const securityLabel =
    critical > 0
      ? t("debug.criticalSummary", { count: String(critical) })
      : warn > 0
        ? t("debug.warningsSummary", { count: String(warn) })
          ...
          <div>
            ...
            ${securitySummary
              ? html`<div class="callout ${securityTone}" style="margin-top: 8px;">
                  ${t("debug.securityAuditLead")} ${securityLabel}${info > 0
                    ? t("debug.infoSummary", { count: String(info) })
                    : ""}. ${t("debug.runForDetails")}
