...
function renderAgentContextCard(
  ...
  return html`
    <section class="card">
      <div class="card-title">${t("ui.agents.context.title")}</div>
      <div class="card-sub">${subtitle}</div>
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t("ui.agents.labels.workspace")}</div>
          <div>
            <button
              type="button"
              class="workspace-link mono"
              @click=${() => onSelectPanel("files")}
              title=${t("ui.agents.toolbar.openFilesTooltip")}
            >
              ${context.workspace}
            </button>
          </div>
        ...
        <div class="agent-kv">
          <div class="label">${t("ui.agents.labels.primaryModel")}</div>
          <div class="mono">${context.model}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("ui.agents.labels.identityName")}</div>
          <div>${context.identityName}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("ui.agents.labels.identityAvatar")}</div>
          <div>${context.identityAvatar}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("ui.agents.labels.skillsFilter")}</div>
          <div>${context.skillsLabel}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("ui.agents.labels.default")}</div>
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(
        params.context,
        t("ui.agents.context.channelsSubtitle"),
      ...
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">${t("ui.agents.channels.subtitle")}</div>
          </div>
        ...
        <div class="muted" style="margin-top: 8px;">
          ${t("ui.agents.channels.lastRefresh", { time: lastSuccessLabel })}
        </div>
        ${params.error
        ...
        ${entries.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">
                ${t("ui.agents.channels.noChannels")}
              </div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${entries.map((entry) => {
                  ...
                  const status = summary.total
                    ? t("ui.agents.channels.status.connected", {
                        connected: String(summary.connected),
                        total: String(summary.total),
                      })
                    : t("ui.agents.channels.status.noAccounts");
                  const configLabel = summary.configured
                    ? t("ui.agents.channels.status.configured", {
                        count: String(summary.configured),
                      })
                    : t("ui.agents.channels.status.notConfigured");
                  const enabled = summary.total
                    ? t("ui.agents.channels.status.enabledCount", { count: String(summary.enabled) })
                    : t("ui.agents.channels.status.disabled");
                          ...
                          ? html`
                              <div>
                                <a
                                  ...
                                  >${t("ui.agents.channels.setupGuide")}</a
                                >
                              </div>
                            `
                          : nothing}
...
export function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(
        params.context,
        t("ui.agents.context.cronSubtitle"),
        params.onSelectPanel,
      )}
      <section class="card">
          <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t("ui.agents.cron.title")}</div>
            <div class="card-sub">${t("ui.agents.cron.subtitle")}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("common.refreshing") : t("common.refresh")}
          </button>
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          ...
          <div class="stat">
            <div class="stat-label">${t("ui.agents.cron.jobsLabel")}</div>
            <div class="stat-value">${params.status?.jobs ?? t("common.na")}</div>
          </div>
          <div class="stat">
            <div class="stat-label">${t("ui.agents.cron.nextWakeLabel")}</div>
    ...
    <section class="card">
      <div class="card-title">${t("ui.agents.cron.jobsCardTitle")}</div>
      <div class="card-sub">${t("ui.agents.cron.jobsCardSubtitle")}</div>
      ${jobs.length === 0
        ? html`<div class="muted" style="margin-top: 16px">${t("ui.agents.cron.noJobs")}</div>`
        : html`
            <div class="list" style="margin-top: 16px;">
              ${jobs.map(
                (job) => html`
...
export function renderAgentFiles(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("ui.agents.files.title")}</div>
          <div class="card-sub">${t("ui.agents.files.subtitle")}</div>
        </div>
        <button
          class="btn btn--sm"
      ...
      ${list
        ? html`<div class="muted mono" style="margin-top: 8px;">
            ${t("ui.agents.files.workspaceLabel")}: <span>${list.workspace}</span>
          </div>`
        : nothing}
      ...
      ${!list
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${t("ui.agents.files.loadHint")}
            </div>
          `
        : files.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">
                ${t("ui.agents.files.noFiles")}
              </div>
            `
          : html`
                          ...
                          ${label}${file.missing
                            ? html`
                                <span class="agent-tab-badge">
                                  ${t("ui.agents.files.missingBadge")}
                                </span>
                              `
                            : nothing}
                        </button>
              ...
              ${!activeEntry
                ? html`
                    <div class="muted" style="margin-top: 16px">
                      ${t("ui.agents.files.selectFileHint")}
                    </div>
                  `
                : html`
                    <div class="agent-file-header" style="margin-top: 14px;">
                      ...
                      <div class="agent-file-actions">
                        <button
                          class="btn btn--sm"
                          title=${t("ui.agents.files.previewTitle")}
                          @click=${(e: Event) => {
                        ...
                        >
                          ${icons.eye} ${t("ui.agents.files.previewLabel")}
                        </button>
                        <button
                          class="btn btn--sm"
                          ?disabled=${!isDirty}
                    ...
                    ${activeEntry.missing
                      ? html`
                          <div class="callout info" style="margin-top: 10px">
                            ${t("ui.agents.files.missingFileCallout")}
                          </div>
                        `
                      : nothing}
                    <label class="field agent-file-field" style="margin-top: 12px;">
                      <span>${t("ui.agents.files.contentLabel")}</span>
                      <textarea
                        ...
                        <div class="md-preview-dialog__header">
                          ...
                          <div class="md-preview-dialog__actions">
                            <button
                              ...
                              title=${t("ui.agents.files.previewFullscreenTitle")}
                              @click=${(e: Event) => {
                                const btn = e.currentTarget as HTMLElement;
                            ...
                            >
                              <span class="when-normal">
                                ${icons.maximize} ${t("ui.agents.files.expandLabel")}
                              </span
                              ><span class="when-fullscreen">
                                ${icons.minimize} ${t("ui.agents.files.collapseLabel")}
                              </span>
                            </button>
                            <button
                              class="btn btn--sm"
                              title=${t("ui.agents.files.editFileTitle")}
                              @click=${(e: Event) => {
                            ...
                            >
                              ${icons.edit} ${t("ui.agents.files.editorLabel")}
                            </button>
                            <button
                              class="btn btn--sm"
                              @click=${(e: Event) => {
                            ...
                            >
                              ${icons.x} ${t("ui.agents.files.closeLabel")}
                            </button>
                          </div>
                        </div>
