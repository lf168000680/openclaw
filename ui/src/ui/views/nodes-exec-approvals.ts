...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  { value: "deny", label: t("nodes.approvals.securityOptions.deny") },
  { value: "allowlist", label: t("nodes.approvals.securityOptions.allowlist") },
  { value: "full", label: t("nodes.approvals.securityOptions.full") },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: t("nodes.approvals.askOptions.off") },
  { value: "on-miss", label: t("nodes.approvals.askOptions.onMiss") },
  { value: "always", label: t("nodes.approvals.askOptions.always") },
];
    ...
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">${t("nodes.approvals.targetSubtitle")}</div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              <option value="gateway" ?selected=${state.target === "gateway"}>
                ${t("nodes.approvals.gateway")}
              </option>
              <option value="node" ?selected=${state.target === "node"}>
                ${t("nodes.approvals.node")}
              </option>
            </select>
          </label>
          ${state.target === "node"
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.approvals.security")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.approvals.securityDefault")
              : t("nodes.approvals.defaultValue", { value: defaults.security })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes.approvals.mode")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t("nodes.approvals.useDefaultWithValue", { value: defaults.security })}
                  </option>`
                : nothing}
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.approvals.ask")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.approvals.askDefault")
              : t("nodes.approvals.defaultValue", { value: defaults.ask })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${t("nodes.approvals.useDefaultWithValue", { value: defaults.ask })}
                  </option>`
                : nothing}
              ${ASK_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.approvals.askFallback")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.approvals.askFallbackDefault")
              : t("nodes.approvals.defaultValue", { value: defaults.askFallback })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes.approvals.fallback")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
            ...
            >
              ${!isDefaults
                ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${t("nodes.approvals.useDefaultWithValue", { value: defaults.askFallback })}
                  </option>`
                : nothing}
              ${SECURITY_OPTIONS.map(
                (option) =>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.approvals.autoAllowSkillClis")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.approvals.autoAllowSkillClisDefault")
              : autoIsDefault
                ? t("nodes.approvals.usingDefault", {
                    value: defaults.autoAllowSkills
                      ? t("nodes.approvals.on")
                      : t("nodes.approvals.off"),
                  })
                : t("nodes.approvals.overrideState", {
                    value: autoEffective ? t("nodes.approvals.on") : t("nodes.approvals.off"),
                  })}
          </div>
        </div>
        <div class="list-meta">
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">${t("nodes.approvals.allowlistTitle")}</div>
        <div class="card-sub">${t("nodes.approvals.allowlistSubtitle")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
      ...
      >
        ${t("nodes.approvals.addPattern")}
      </button>
    </div>
    <div class="list" style="margin-top: 12px;">
      ${entries.length === 0
        ? html` <div class="muted">${t("nodes.approvals.noAllowlistEntries")}</div> `
    ...
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">
          ${entry.pattern?.trim() ? entry.pattern : t("nodes.approvals.newPattern")}
        </div>
        <div class="list-sub">${t("nodes.approvals.lastUsed", { value: lastUsed })}</div>
        ${lastCommand ? html`<div class="list-sub mono">${lastCommand}</div>` : nothing}
        ${lastPath ? html`<div class="list-sub mono">${lastPath}</div>` : nothing}
      </div>
      <div class="list-meta">
        <label class="field">
          <span>${t("nodes.approvals.pattern")}</span>
          <input
            type="text"
            .value=${entry.pattern ?? ""}
            ?disabled=${state.disabled}
