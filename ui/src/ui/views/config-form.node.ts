  ...
  return html`
    <button
      ...
      title=${state.canReveal
        ? state.isRevealed
          ? t("ui.config.hideValue")
          : t("ui.config.revealValue")
        : t("ui.config.revealValueUnavailable")}
...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">${t("ui.config.unsupportedSchemaNode")}</div>
    </div>`;
  }
  if (
    criteria &&
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error">${t("ui.config.unsupportedType", { type })}</div>
    </div>
  `;
}
...
function renderTextInput(params: {
  ...
  const placeholder = effectiveRedacted
      ? isStructuredSecretRef
        ? rawAvailable
        ? t("ui.config.structuredSecretRefRaw")
        : t("ui.config.structuredSecretRefFile")
      : REDACTED_PLACEHOLDER
    : (hint?.placeholder ??
      (schema.default !== undefined
        ? t("ui.config.defaultValue", { value: formatUnknownText(schema.default) })
        : ""));
        ...
        ${schema.default !== undefined
          ? html`
              <button
                ...
                title=${t("ui.config.resetToDefault")}
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${t("ui.config.selectPlaceholder")}</option>
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">${t("ui.config.unsupportedArraySchema")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count">${t("ui.config.arrayCount", { count: arr.length })}</span>
        <button
          type="button"
      ...
      ${arr.length === 0
        ? html` <div class="cfg-array__empty">${t("ui.config.noItemsYet")}</div> `
        : html`
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">${t("ui.config.customEntries")}</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
        ...
        >
          ...
          ${t("ui.config.addEntry")}
        </button>
      </div>

      ${visibleEntries.length === 0
        ? html` <div class="cfg-map__empty">${t("ui.config.noCustomEntries")}</div> `
        : html`
