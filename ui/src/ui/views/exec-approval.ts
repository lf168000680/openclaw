...
function formatRemaining(ms: number): string {
  ...
  if (totalSeconds < 60) {
    return t("execApproval.seconds", { count: String(totalSeconds) });
  }
  const minutes = Math.floor(totalSeconds / 60);
  if (minutes < 60) {
    return t("execApproval.minutes", { count: String(minutes) });
  }
  const hours = Math.floor(minutes / 60);
  return t("execApproval.hours", { count: String(hours) });
}
        ...
        <div class="exec-approval-header">
          ...
          ${queueCount > 1
            ? html`<div class="exec-approval-queue">
                ${t("execApproval.pendingCount", { count: String(queueCount) })}
              </div>`
            : nothing}
        </div>
